...
# ============================================================================
# Optimized Read-Only Search Tests (TEST-029)
# These tests use the session-scoped pre_indexed_server fixture
# to avoid redundant indexing. They only perform READ operations.
# ============================================================================

@pytest.mark.e2e
@pytest.mark.asyncio
async def test_semantic_search_authentication(pre_indexed_server):
    """Test: Semantic search for authentication code.

    Uses pre-indexed server to avoid redundant indexing.
    This test is READ-ONLY - it only searches, doesn't modify data.
    """
    server = pre_indexed_server

    # Search for authentication-related code
    search_result = await server.search_code(
        query="authentication function verify password",
    ...
    result_names = [r.get("unit_name", "").lower() for r in search_result["results"]]
    assert any("authenticate" in name or "verify" in name or "password" in name
               for name in result_names)


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_semantic_search_database(pre_indexed_server):
    """Test: Semantic search for database code.
    ...
    # Should find DatabaseConnection class or related code
    found_relevant = False
    for result in results:
        unit_name = result.get("unit_name", "").lower()
        if "database" in unit_name or "connection" in unit_name or "query" in unit_name:
            found_relevant = True
            break
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_semantic_search_api_handlers(pre_indexed_server):
    """Test: Semantic search for API handler code.
    ...
    search_result = await server.search_code(
        query="API request handler HTTP",
        project_name="shared-test-project",
        search_mode="semantic",
        limit=5
    )
    ...
    for result in results:
        unit_name = result.get("unit_name", "").lower()
        if "request" in unit_name or "api" in unit_name or "handler" in unit_name:
            found_relevant = True
            break

    assert found_relevant, "Should find API handler code units"


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_hybrid_search_mode(pre_indexed_server):
    """Test: Hybrid search mode (BM25 + semantic).
    ...
    # Hybrid search should find relevant code
    result_names = [r.get("unit_name", "").lower() for r in results]
    assert any("user" in name or "authenticate" in name for name in result_names)


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_search_performance(pre_indexed_server):
    """Test: Search performance with pre-indexed data.

    Uses pre-indexed server to avoid redundant indexing.
    This test is READ-ONLY - it only searches, doesn't modify data.

    Verifies that search operations complete quickly.
    """
    server = pre_indexed_server
    ...
    # Search should be fast (target: < 1000ms for this dataset)
    assert search_time < 1.0, f"Search took {search_time*1000:.0f}ms, expected < 1000ms"


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_get_indexed_files(pre_indexed_server):
    """Test: Get list of indexed files.

    Uses pre-indexed server to avoid redundant indexing.
    This test is READ-ONLY - it only queries, doesn't modify data.
    """
    server = pre_indexed_server
    ...
    assert len(files) >= 4  # Should have at least 4 Python files

    # Verify file names
    file_names = [Path(f.get("file_path", "")).name for f in files]
    expected_files = {"auth.py", "database.py", "api.py", "utils.py", "main.py"}
    found_files = set(file_names)
    assert len(expected_files.intersection(found_files)) >= 4, \
        f"Expected at least 4 of {expected_files}, found {found_files}"


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_multiple_searches_same_session(pre_indexed_server):
    """Test: Multiple sequential searches on pre-indexed data.

    Uses pre-indexed server to avoid redundant indexing.
    This test is READ-ONLY - it only searches, doesn't modify data.

    Demonstrates the performance benefit of session-scoped fixtures.
    """
    server = pre_indexed_server
    ...
    assert avg_time < 0.5, f"Average search time {avg_time*1000:.0f}ms, expected < 500ms"
