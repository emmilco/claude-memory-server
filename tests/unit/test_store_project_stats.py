...
class TestSQLiteProjectStats:
    """Test project statistics methods in SQLite store."""

    @pytest.mark.asyncio
    async def test_get_all_projects_empty(self, tmp_path):
        """Test get_all_projects with no projects."""
        import sqlite3
    ...
    @pytest.mark.asyncio
    async def test_get_all_projects(self, tmp_path):
        """Test get_all_projects returns unique projects."""
        import sqlite3
        ...
        # Insert test data
        conn.execute("""
            INSERT INTO memories (id, content, category, context_level, scope, project_name, importance, embedding_model, created_at, updated_at, tags, metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ("id1", "content", "code", "FILE", "project", "project1", 0.5, "model", "2025-01-01", "2025-01-01", "[]", "{}"))

        conn.execute("""
            INSERT INTO memories (id, content, category, context_level, scope, project_name, importance, embedding_model, created_at, updated_at, tags, metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ("id2", "content", "code", "FILE", "project", "project2", 0.5, "model", "2025-01-01", "2025-01-01", "[]", "{}"))

        conn.execute("""
            INSERT INTO memories (id, content, category, context_level, scope, project_name, importance, embedding_model, created_at, updated_at, tags, metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ("id3", "content", "code", "FILE", "project", "project1", 0.5, "model", "2025-01-01", "2025-01-01", "[]", "{}"))
    ...
    @pytest.mark.asyncio
    async def test_get_project_stats(self, tmp_path):
        """Test get_project_stats returns correct statistics."""
        import sqlite3
        import json
        ...
        # Insert test data
        metadata1 = json.dumps({"file_path": "/path/to/file1.py"})
        metadata2 = json.dumps({"file_path": "/path/to/file2.py"})

        conn.execute("""
            INSERT INTO memories (id, content, category, context_level, scope, project_name, importance, embedding_model, created_at, updated_at, tags, metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ("id1", "content", "code", "FILE", "project", "test-project", 0.5, "model", "2025-01-01T10:00:00", "2025-01-01T10:00:00", "[]", metadata1))

        conn.execute("""
            INSERT INTO memories (id, content, category, context_level, scope, project_name, importance, embedding_model, created_at, updated_at, tags, metadata)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ("id2", "content", "code", "FUNCTION", "project", "test-project", 0.5, "model", "2025-01-02T10:00:00", "2025-01-02T10:00:00", "[]", metadata2))
