...
class TestFileExtensionRecognition:
    ...
    @pytest.mark.parametrize(
        "extension,sample_code,expected_lang",
        [
    ...
    def test_extension_recognized(
        self, tmp_path, extension, sample_code, expected_lang
    ):
...
class TestClassExtraction:
    ...
    @pytest.mark.parametrize(
        "extension,sample_code,class_name",
        [
    ...
    def test_class_extraction(self, tmp_path, extension, sample_code, class_name):
        ...
        classes = [
            u
            for u in units
            if get_unit_attr(u, "unit_type") == "class"
...
class TestFunctionExtraction:
    ...
    @pytest.mark.parametrize(
        "extension,sample_code,function_name",
        [
    ...
    def test_function_extraction(self, tmp_path, extension, sample_code, function_name):
        ...
        functions = [
            u
            for u in units
            if get_unit_attr(u, "unit_type") == "function"
            and function_name in get_unit_attr(u, "name").lower()
        ]
        assert (
            len(functions) > 0
        ), f"Should extract {function_name} function/method for {extension}"
...
class TestMultipleSemanticUnits:
    ...
    @pytest.mark.parametrize(
        "extension,sample_code,min_units",
        [
    ...
    def test_multiple_units_extracted(
        self, tmp_path, extension, sample_code, min_units
    ):
...
class TestUnitMetadata:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ".js", JAVASCRIPT_SAMPLE, "JavaScript", id="javascript-metadata"
            ),
        ],
    )
    def test_unit_metadata_correct(
        self, tmp_path, extension, sample_code, expected_lang
    ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ".js", JAVASCRIPT_SAMPLE, "JavaScript", id="javascript-file-path"
            ),
        ],
    )
    def test_result_file_path_correct(
        self, tmp_path, extension, sample_code, expected_lang
    ):
