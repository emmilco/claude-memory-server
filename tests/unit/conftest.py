...
- Session-scoped fixtures for immutable resources (configs, static data)
- Module-scoped fixtures for read-only resources
- Function-scoped fixtures for mutable resources

Scope Guidelines:
- Use session scope for fixtures that create immutable objects shared across all tests
- Use module scope for fixtures that create immutable objects shared within a module
...
def session_config():
    """Session-scoped config for read-only tests.
