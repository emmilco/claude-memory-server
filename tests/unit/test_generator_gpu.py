...
class TestGeneratorGpuSupport:
    ...
    def test_device_determination_gpu_enabled_available(self):
        ...
        with (
            patch("src.embeddings.generator.get_optimal_device", return_value="cuda"),
            patch(
                "src.embeddings.generator.get_gpu_info",
                return_value={
    ...
    def test_device_determination_gpu_unavailable(self):
        ...
        with (
            patch("src.embeddings.generator.get_optimal_device", return_value="cpu"),
    ...
    def test_model_load_with_gpu(self):
        ...
        with (
            ...
            patch(
                "src.embeddings.generator.SentenceTransformer", return_value=mock_model
            ),
            ...
            mock_torch.cuda.set_per_process_memory_fraction.assert_called_once_with(
                0.7, 0
            )
            ...
            ), "Model should be the loaded SentenceTransformer"
    ...
    def test_model_load_gpu_fallback_to_cpu(self):
        ...
        with (
            ...
            ), "Device should initially be cuda before fallback"
            ...
            ), "Generator device should fall back to cpu after GPU error"
            assert (
                generator.model is not None
            ), "Model should be initialized despite GPU error"
            assert (
                generator.model == mock_model
            ), "Model should be the loaded SentenceTransformer"
    ...
    def test_benchmark_includes_device(self):
        ...
        with (
            ...
            patch.dict("sys.modules", {"torch": mock_torch}),
            patch(
                "src.embeddings.generator.RustBridge.batch_normalize"
            ) as mock_normalize,
        ):
