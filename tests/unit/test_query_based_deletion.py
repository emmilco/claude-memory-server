...
class TestQdrantStoreDeleteByFilter:
    ...
    async def test_delete_by_filter_invalid_max_count(self, mock_config):
        ...
        # Test max_count < 1
        with pytest.raises(
            ValidationError, match="max_count must be between 1 and 1000"
        ):
    ...
    async def test_delete_by_filter_error_handling(self, mock_config):
        ...
        with (
            ...
            with pytest.raises(
                StorageError, match="Failed to delete memories by filter"
            ):
...
class TestServerDeleteMemoriesByQuery:
    ...
    async def test_dry_run_mode(self, mock_config, sample_memories):
        ...
        result = await server.delete_memories_by_query(
            project_name="project1", dry_run=True
        )
    ...
    async def test_actual_deletion(self, mock_config):
        ...
        with patch.object(server.store, "delete_by_filter", return_value=mock_result):
            result = await server.delete_memories_by_query(
                project_name="project1", category="code", dry_run=False
            )
    ...
    async def test_read_only_mode_check(self, mock_config):
        ...
        with pytest.raises(
            ReadOnlyError, match="Cannot delete memories in read-only mode"
        ):
            await server.delete_memories_by_query(project_name="test", dry_run=False)
    ...
    async def test_dry_run_with_high_importance_warning(self, mock_config):
        ...
        with patch.object(
            server.store, "list_memories", return_value=[high_importance_memory]
        ):
            result = await server.delete_memories_by_query(
                max_importance=1.0, dry_run=True
            )
    ...
    @pytest.mark.asyncio
    async def test_dry_run_with_multiple_projects_warning(
    ...
    async def test_stats_update_on_deletion(self, mock_config):
        ...
        with patch.object(server.store, "delete_by_filter", return_value=mock_result):
            await server.delete_memories_by_query(
                project_name="project1", dry_run=False
            )
    ...
    async def test_error_propagation(self, mock_config):
        ...
        with patch.object(
            server.store, "delete_by_filter", side_effect=Exception("Database error")
        ):
            with pytest.raises(
                StorageError, match="Failed to delete memories by query"
            ):
                await server.delete_memories_by_query(
                    project_name="test", dry_run=False
                )
    ...
    async def test_string_enum_conversion(self, mock_config):
        ...
        with patch.object(
            server.store, "delete_by_filter", return_value=mock_result
        ) as mock_delete:
