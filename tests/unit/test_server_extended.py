...
class TestCodeSearch:
    ...
    async def test_search_code_basic(self, server, small_test_project):
        ...
        search_results = await server.search_code(
            query="test function", project_name="test-project", limit=5
        )
    ...
    async def test_search_code_with_filters(self, server, small_test_project):
        ...
        index_result = await server.index_codebase(
            directory_path=str(small_test_project), project_name="test-project"
        )
        ...
        results = await server.search_code(
            query="test", project_name="test-project", language="python", limit=10
        )
        ...
        for result in results["results"]:
            ...
            assert (
                result["language"].lower() == "python"
            ), f"Expected Python but got {result['language']}"
    ...
    async def test_index_codebase_nonexistent_path(self, server):
        ...
        with pytest.raises(StorageError, match="does not exist"):
            await server.index_codebase(
                directory_path="/nonexistent/path/xyz", project_name="test"
            )
...
class TestMemoryOperations:
    ...
    async def test_retrieve_with_min_importance(self, server):
        ...
        await server.store_memory(
            content="Low importance", category="fact", scope="global", importance=0.3
        )

        await server.store_memory(
            content="High importance", category="fact", scope="global", importance=0.9
        )

        # Retrieve with min_importance filter
        results = await server.retrieve_memories(
            query="importance", min_importance=0.7, limit=10
        )
    ...
    async def test_retrieve_with_category_filter(self, server):
        ...
        await server.store_memory(
            content="Preference memory", category="preference", scope="global"
        )

        await server.store_memory(
            content="Fact memory", category="fact", scope="global"
        )

        # Retrieve only preferences
        results = await server.retrieve_memories(
            query="memory", category="preference", limit=10
        )
...
class TestReadOnlyMode:
    ...
    async def test_readonly_blocks_store(self, readonly_server):
        ...
        with pytest.raises(ReadOnlyError):
            await readonly_server.store_memory(
                content="This should fail", category="fact", scope="global"
            )
    ...
    async def test_readonly_allows_retrieve(self, readonly_server):
        ...
        results = await readonly_server.retrieve_memories(query="test", limit=5)
...
class TestSpecializedRetrieval:
    ...
    async def test_retrieve_preferences_filters_correctly(self, server):
        ...
        await server.store_memory(
            content="I prefer Python", category="preference", scope="global"
        )

        await server.store_memory(
            content="This is a fact", category="fact", scope="global"
        )

        # Get preferences
        results = await server.retrieve_preferences(query="Python fact", limit=10)
    ...
    async def test_retrieve_project_context_with_project_name(self, server):
        ...
        for result in results["results"]:
            assert (
                "project_name" in result["memory"]
            ), "Result missing required 'project_name' field"
            assert (
                result["memory"]["project_name"] == "project-a"
...
class TestErrorHandling:
    ...
    async def test_store_minimal_content(self, server):
        ...
        result = await server.store_memory(content="x", category="fact", scope="global")
    ...
    async def test_retrieve_with_limit(self, server, limit):
        ...
        ), f"Got {len(results['results'])} results but limit was {limit}"
    ...
    async def test_get_status_structure(self, server):
        ...
        assert "read_only_mode" in status
        assert isinstance(
            status.get("memory_count", status.get("total_memories", 0)), int
        )
...
"""
        ...
        search_results = await server.find_similar_code(
            code_snippet=code_snippet, project_name="test-project", limit=5
        )
    ...
    async def test_find_similar_code_empty_snippet(self, server):
        ...
        with pytest.raises(ValidationError):
            await server.find_similar_code(code_snippet="", project_name="test-project")
    ...
    @pytest.mark.asyncio
    async def test_find_similar_code_with_language_filter(
        self, server, small_test_project
    ):
    ...
    async def test_find_similar_code_no_results(self, server):
        ...
        results = await server.find_similar_code(
            code_snippet=code_snippet, project_name="nonexistent-project", limit=5
        )
    ...
    async def test_find_similar_code_result_format(self, server, small_test_project):
        ...
        results = await server.find_similar_code(
            code_snippet=code_snippet, project_name="test-project", limit=3
        )
    ...
    @pytest.mark.asyncio
    async def test_find_similar_code_high_similarity_interpretation(
        self, server, small_test_project
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_find_similar_code_with_file_pattern(
        self, server, small_test_project
    ):
