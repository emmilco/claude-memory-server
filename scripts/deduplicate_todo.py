...
def main():
    ...
    output_path = Path(
        "/Users/elliotmilco/Documents/GitHub/claude-memory-server/TODO_NEW.md"
    )
    ...
    # Parse all tasks
    tasks: Dict[str, List[List[str]]] = defaultdict(
        list
    )  # task_id -> list of occurrences
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            "- Priority: Tasks marked 🔴 are critical, 🟡 are high priority, 🟢 are medium\n\n"
        )
        f.write("---\n\n")
        ...
        for prefix in ["BUG", "REF", "PERF"]:
            for task_id, task_lines in categories[prefix]:
                first_line = task_lines[0]
                if any(marker in first_line for marker in ["HIGH", "🟡"]) and not any(
                    marker in first_line for marker in ["CRITICAL", "🔴"]
                ):
                    for line in task_lines:
                        f.write(line)
                    f.write("\n")
        ...
        for task_id, task_lines in categories["BUG"]:
            ...
            if not any(
                marker in first_line for marker in ["CRITICAL", "🔴", "HIGH", "🟡"]
            ):
                for line in task_lines:
                    f.write(line)
                f.write("\n")
    ...
    print(
        f"Reduction: {len(lines) - new_lines} lines ({100*(len(lines)-new_lines)/len(lines):.1f}%)"
    )
    print("=" * 60)
