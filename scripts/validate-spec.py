    ...
    for feature in features:
        for field in required_feature_fields:
            if field not in feature:
                issues.append(
                    f"Feature {feature.get('id', 'UNKNOWN')} missing field: {field}"
                )
...
def validate_requirements(spec: Dict) -> Tuple[bool, Dict]:
    ...
    for feature in features:
        ...
        feature_stats = {"total": 0, "passing": 0, "failing": 0}
        ...
        for req in requirements:
            ...
            for field in required_req_fields:
                if field not in req:
                    issues.append(
                        f"Requirement {req.get('id', 'UNKNOWN')} missing field: {field}"
                    )
            ...
            if req_type not in valid_types:
                issues.append(
                    f"Requirement {req.get('id', 'UNKNOWN')} has invalid type: {req_type}"
                )
            else:
            ...
            if status not in valid_statuses:
                issues.append(
                    f"Requirement {req.get('id', 'UNKNOWN')} has invalid status: {status}"
                )
            else:
            ...
            if not test_refs:
                ...
                issues.append(
                    f"Requirement {req.get('id', 'UNKNOWN')} has no test references"
                )
            ...
            if not all(k in acceptance for k in ["given", "when", "then"]):
                issues.append(
                    f"Requirement {req.get('id', 'UNKNOWN')} missing acceptance criteria (given/when/then)"
                )
        ...
        for req in requirements:
            ...
            for test_ref in test_refs:
                ...
                if not test_path.exists():
                    issues.append(
                        f"Test file not found: {test_file} (referenced by {req.get('id', 'UNKNOWN')})"
                    )
...
def validate_compliance(spec: Dict) -> Tuple[bool, Dict]:
    ...
    if discrepancies:
        for key, values in discrepancies.items():
            issues.append(
                f"Compliance mismatch for {key}: claimed {values['claimed']}, actual {values['actual']}"
            )
...
def display_results(spec: Dict, results: Dict, verbose: bool = False):
    ...
    for check, result in results.items():
        if check == "requirements":
            ...
            if passed:
                print(
                    f"  {GREEN}✓{RESET} Requirements: All {stats['total']} requirements valid"
                )
            else:
                print(
                    f"  {RED}✗{RESET} Requirements: {len(result['issues'])} issues found"
                )
                all_passed = False
        else:
            passed = result["passed"]
            if passed:
                ...
                print(
                    f"  {RED}✗{RESET} {check.title()}: {len(result['issues'])} issues"
                )
                all_passed = False
    ...
    if "requirements" in results:
        ...
        if verbose and stats["by_feature"]:
            ...
            for feature_id, feature_stats in stats["by_feature"].items():
                ...
                print(
                    f"  {feature_id}: {status_color}{passing}/{total} ({pct:.0f}%){RESET}"
                )
