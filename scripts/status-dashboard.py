...
def count_tasks(file_path: Path) -> Tuple[int, List[str]]:
    ...
    for line in lines:
        ...
        if stripped.startswith("## ") and (
            "template" in stripped or "example" in stripped
        ):
...
def get_test_status() -> Dict[str, any]:
    ...
    try:
        ...
        return {"total": 0, "status": "timeout", "message": "Test collection timed out"}
    except Exception as e:
        return {"total": 0, "status": "error", "message": f"Error: {str(e)}"}
...
def get_coverage_status() -> Dict[str, any]:
    ...
    if coverage_file.exists():
        ...
        return {"overall": "~60%", "core": "~71%", "target": "80%", "status": "partial"}
    else:
        return {
            "overall": "unknown",
            "core": "unknown",
...
def get_git_status() -> Dict[str, any]:
    ...
    try:
        ...
        ahead_behind = (
            result.stdout.strip().split() if result.returncode == 0 else ["0", "0"]
        )
...
def display_dashboard():
    ...
    # Header
    print_header("Claude Memory RAG Server - Status Dashboard")
    ...
    capacity_color = (
        GREEN if capacity_pct < 80 else YELLOW if capacity_pct < 100 else RED
    )

    print(f"  TODO:          {YELLOW}{task_status['todo']:3d}{RESET} tasks planned")
    print(
        f"  IN PROGRESS:   {capacity_color}{task_status['in_progress']:3d}{RESET}/{task_status['capacity_max']} ({capacity_pct:.0f}% capacity)"
    )
    print(
        f"  REVIEW:        {BLUE}{task_status['review']:3d}{RESET} tasks awaiting approval"
    )
    print(
        f"  COMPLETED:     {GREEN}{task_status['completed_recent']:3d}{RESET} tasks in recent history"
    )
    ...
    if task_status["in_progress"] > 0:
        ...
        if len(task_status["in_progress_ids"]) > 5:
            print(
                f"                ... and {len(task_status['in_progress_ids']) - 5} more"
            )
    ...
    if git_status["ahead"] > 0 or git_status["behind"] > 0:
        print(
            f"  Sync:          {YELLOW}{git_status['ahead']} ahead, {git_status['behind']} behind origin{RESET}"
        )
    ...
    if qdrant_status["status"] == "running":
        print(
            f"  Collections:   {CYAN}{qdrant_status['collections']:3d}{RESET} collections"
        )
    ...
    print("  Run tests:     pytest tests/ -n auto -v")
    print("  Coverage:      pytest tests/ --cov=src --cov-report=html")
    print("  Verify:        python scripts/verify-complete.py")
    print("  Setup check:   python scripts/setup.py")
    print("  Health check:  python -m src.cli health")
