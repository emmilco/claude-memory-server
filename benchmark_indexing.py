...
class PerformanceBenchmark:
    ...
    async def benchmark_indexing(self, directory: Path) -> dict:
        ...
        metrics = {
            ...
            "files_per_sec": result["indexed_files"] / total_time if total_time > 0 else 0,
            "units_per_sec": result["total_units"] / total_time if total_time > 0 else 0,
            "avg_time_per_file_ms": (total_time * 1000) / result["indexed_files"] if result["indexed_files"] > 0 else 0,
        }
    ...
    async def benchmark_search(self, queries: list[str]) -> dict:
        ...
        for query in queries:
            ...
            print(f"  (Embed: {embed_time*1000:.2f}ms, Search: {search_time*1000:.2f}ms)\n")
    ...
    async def analyze_indexed_code(self) -> dict:
        ...
        for fname, count in list(sorted(by_file.items(), key=lambda x: x[1], reverse=True))[:10]:
            print(f"  {fname}: {count}")
    ...
    async def cleanup(self):
        ...
        if self.store.client:
            try:
                ...
                logger.info(f"Deleted benchmark collection: {self.config.qdrant_collection_name}")
