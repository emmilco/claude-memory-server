...
class TestCoordinator:
    ...
    def collect_results(self):
        ...
        if len(self.agent_results) == 0:
            logger.warning(
                "No agent results found - check that tests have been executed"
            )
        ...
        for bug in agent_result.get("bugs_found", []):
            ...
            if bug_id:
                existing = next(
                    (
                        b
                        for b in self.consolidated_bugs
                        if (b.get("id") == bug_id or b.get("bug_id") == bug_id)
                    ),
                    None,
                )
            ...
            if not existing and description:
                existing = next(
                    (
                        b
                        for b in self.consolidated_bugs
                        if b.get("description", "").strip() == description
                    ),
                    None,
                )
            ...
            if existing:
                ...
                if self._severity_rank(bug.get("severity")) > self._severity_rank(
                    existing.get("severity")
                ):
                    existing["severity"] = bug.get("severity")

                logger.debug(
                    f"Merged duplicate bug {bug_id or description[:50]} from agent {agent_id}"
                )
            else:
                ...
                normalized_bug = {
                    ...
                    "found_in_tests": [bug.get("test_id")]
                    if bug.get("test_id")
                    else [],
                }
    ...
    def calculate_summary(self):
        ...
        automated_pass_rate = (
            (automated_passed / automated_tests * 100) if automated_tests > 0 else 0
        )
        ...
        self.final_report["summary"] = {
            ...
            "critical_bugs": len(
                [b for b in self.consolidated_bugs if b.get("severity") == "CRITICAL"]
            ),
            "high_bugs": len(
                [b for b in self.consolidated_bugs if b.get("severity") == "HIGH"]
            ),
            "medium_bugs": len(
                [b for b in self.consolidated_bugs if b.get("severity") == "MEDIUM"]
            ),
            "low_bugs": len(
                [b for b in self.consolidated_bugs if b.get("severity") == "LOW"]
            ),
        }
    ...
    def assess_production_readiness(self):
        ...
        score_components = {
            ...
            "high_bugs": 20
            if criteria["max_3_high_bugs"]
            else max(0, 20 - (high_bugs - 3) * 5),
            "pass_rate": min(25, pass_rate / 4),
            "failures": 15
            if criteria["zero_test_failures"]
            else max(0, 15 - failed * 3),
        ...
        self.final_report["production_readiness"] = {
            ...
            "blockers": self._identify_blockers(
                criteria, critical_bugs, high_bugs, failed, errors
            ),
        }

    def _identify_blockers(
        self, criteria: Dict, critical: int, high: int, failed: int, errors: int
    ) -> List[str]:
    ...
    def generate_recommendation(
        ...
        if critical > 0:
            return (
                f"❌ BLOCKED - {critical} CRITICAL bug(s) must be fixed before release"
            )
...
def main():
    ...
    parser.add_argument(
        "--aggregate", action="store_true", help="Aggregate results from all agents"
    )
    parser.add_argument(
        "--results-dir", default="/results", help="Directory containing agent results"
    )
    parser.add_argument(
        "--output-dir",
