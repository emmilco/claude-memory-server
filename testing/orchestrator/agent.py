...
class TestAgent:
        ...
        try:
            ...
            if test_result.get("bugs_found"):
                for bug in test_result["bugs_found"]:
                    ...
                    if not any(
                        b.get("id") == bug_id or b.get("bug_id") == bug_id
    ...
    def calculate_summary(self):
        ...
        manual = len(
            [t for t in self.results["tests"] if t["status"] == "MANUAL_REQUIRED"]
        )
        ...
        bugs_by_severity = {
            ...
            "HIGH": len(
                [b for b in self.results["bugs_found"] if b.get("severity") == "HIGH"]
            ),
            "MEDIUM": len(
                [b for b in self.results["bugs_found"] if b.get("severity") == "MEDIUM"]
            ),
            "LOW": len(
                [b for b in self.results["bugs_found"] if b.get("severity") == "LOW"]
            ),
        }
        ...
        self.results["summary"] = {
            ...
            "estimated_duration_minutes": self.section_config.get(
                "estimated_duration_minutes", 0
            ),
            "critical_bugs_verified": self.section_config.get(
                "critical_bugs_to_verify", []
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--assignments",
        default="/app/test_assignments.json",
