...
class TestExecutor:
    ...
    def __init__(self, project_root: str = "/app"):
        ...
        try:
            ...
            from testing.orchestrator.test_implementations import (
                PerformanceTests, SecurityTests, ErrorHandlingTests, ConfigurationTests
            )
    ...
    def _execute_installation_test(self, test_id: str) -> Dict[str, Any]:
        ...
        if test_id in tests:
            ...
            result['notes'] = f'Test {test_id} requires manual execution - see TEST-006_e2e_test_plan.md'
    ...
    def _execute_code_search_test(self, test_id: str) -> Dict[str, Any]:
        ...
        return self._execute_automated_test(test_id, {
    ...
    def _run_command(self, cmd: List[str], timeout: int = 30, capture_output: bool = True) -> Tuple[int, str, str]:
    ...
    def _test_setup_wizard(self, result: Dict):
        ...
        if not setup_file.exists():
            result['status'] = 'FAIL'
            result['notes'] = 'setup.py not found in repository - automated setup unavailable'
        ...
        try:
            ...
            if missing_components:
                result['status'] = 'FAIL'
                result['notes'] = f'setup.py missing critical components: {", ".join(missing_components)}'
                result['bugs_found'].append({
                    'bug_id': 'BUG-NEW-001B',
                    'severity': 'MEDIUM',
                ...
                result['status'] = 'MANUAL_REQUIRED'
                result['notes'] = 'setup.py exists with required components. Interactive wizard requires manual testing - verify all wizard steps work correctly, error handling is clear, completion time is 2-5 minutes, and user can recover from interruptions.'
        except Exception as e:
            result['status'] = 'ERROR'
    ...
    def _test_sqlite_backend(self, result: Dict):
        ...
        if config_file.exists():
            ...
            if 'Literal["qdrant"]' in config_content or 'storage_backend: Literal["qdrant"]' in config_content:
                result['status'] = 'PASS'
                result['notes'] = (
        ...
        try:
            ...
            has_deprecation_note = 'deprecated' in store_content.lower() or 'qdrant' in store_content.lower()

            result['status'] = 'PASS'
            if has_deprecation_note:
                result['notes'] = 'SQLite backend available (correctly marked as deprecated) - Qdrant is now the primary backend'
            else:
                result['notes'] = 'SQLite backend available but may be missing deprecation warnings'
                result['bugs_found'].append({
                    'bug_id': 'BUG-NEW-003B',
                    'severity': 'LOW',
    ...
    def _test_manual_install(self, result: Dict):
        ...
        if not req_file.exists():
            result['status'] = 'FAIL'
            result['notes'] = 'requirements.txt not found - manual installation impossible'
            result['bugs_found'].append({
                'bug_id': 'BUG-NEW-004',
                'severity': 'CRITICAL',
        ...
        try:
            ...
            critical_deps = ['qdrant-client', 'sentence-transformers', 'mcp', 'watchdog']
            missing_deps = [dep for dep in critical_deps if dep not in req_content.lower()]

            if missing_deps:
                result['status'] = 'FAIL'
                result['notes'] = f'requirements.txt missing critical dependencies: {", ".join(missing_deps)}'
                result['bugs_found'].append({
                    'bug_id': 'BUG-NEW-004B',
                    'severity': 'HIGH',
                ...
                if readme.exists():
                    ...
                    has_install_instructions = 'pip install' in readme_content and 'requirements.txt' in readme_content
                    if has_install_instructions:
                        result['status'] = 'PASS'
                        result['notes'] = 'Manual installation files present - requirements.txt complete and README has installation instructions'
                    else:
                        result['status'] = 'PASS'
                        result['notes'] = 'requirements.txt complete but README may be missing installation instructions'
                        result['bugs_found'].append({
                            'bug_id': 'BUG-NEW-004C',
                            'severity': 'LOW',
                    ...
                    result['status'] = 'PASS'
                    result['notes'] = 'requirements.txt exists and appears complete, but README not found'
        except Exception as e:
            result['status'] = 'ERROR'
    ...
    def _test_docker_qdrant(self, result: Dict):
        ...
        try:
            ...
            if missing_keys:
                result['notes'] = f'docker-compose.yml exists but may be incomplete - missing: {", ".join(missing_keys)}'
                result['bugs_found'].append({
                    'bug_id': 'BUG-NEW-005B',
                    'severity': 'MEDIUM',
        ...
        if returncode == 0 and 'OK' in stdout:
            result['status'] = 'PASS'
            result['notes'] = f'Qdrant accessible at {self.qdrant_url} and docker-compose.yml present'
        else:
            result['status'] = 'FAIL'
            result['notes'] = f'Qdrant not accessible at {self.qdrant_url}: {stderr}. Ensure Docker is running and Qdrant container is started.'
            result['bugs_found'].append({
                'bug_id': 'BUG-NEW-005C',
                'severity': 'HIGH',
    ...
    def _test_health_check(self, result: Dict):
        ...
        if self._is_success_with_warnings(returncode, stdout, stderr):
            ...
            if len(found_components) >= 2:  # At least some health checks present
                result['status'] = 'PASS'
                result['notes'] = f'Health check command executed successfully - checked: {", ".join(found_components)}'
            else:
                result['status'] = 'PASS'
                result['notes'] = f'Health check command ran but output format may be incomplete. Output: {stdout[:300]}'
                result['bugs_found'].append({
                    'bug_id': 'BUG-NEW-006',
                    'severity': 'LOW',
            ...
            result['status'] = 'FAIL'
            result['notes'] = f'Health check failed with exit code {returncode}: {stderr[:500]}'
            result['bugs_found'].append({
                'bug_id': 'BUG-NEW-006B',
                'severity': 'MEDIUM',
        ...
        try:
            ...
            with open(mcp_server) as f:
                ...
                if has_main and has_mcp_tools:
                    result['status'] = 'MANUAL_REQUIRED'
                    result['notes'] = 'MCP server file exists with proper structure. MANUAL TEST REQUIRED: Register with Claude Code using: claude mcp add --transport stdio --scope user claude-memory-rag -- python src/mcp_server.py'
                else:
                    result['status'] = 'FAIL'
                    result['notes'] = 'MCP server file exists but may be missing main entry point or tool definitions'
                    result['bugs_found'].append({
                        'bug_id': 'BUG-NEW-007B',
                        'severity': 'HIGH',
    ...
    def _test_upgrade_path(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'MANUAL TEST REQUIRED: Upgrade from v3.x to v4.0 requires testing data migration, config preservation, and no data loss. Check CHANGELOG.md for migration guide.'

    def _test_uninstall(self, result: Dict):
        """Test INST-009: Clean uninstallation"""
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'MANUAL TEST REQUIRED: Verify clean uninstall removes ~/.claude-rag/, stops Qdrant container, removes MCP registration, and leaves no orphaned files.'
    ...
    def _test_config_files(self, result: Dict):
        ...
        if env_example.exists():
            ...
            notes.append('No .env.example (system may use environment variables or defaults)')

        if readme.exists():
            readme_content = readme.read_text()
            has_config_docs = 'configuration' in readme_content.lower() or 'environment' in readme_content.lower()
            if has_config_docs:
        ...
        result['notes'] = ' | '.join(notes) if notes else 'Configuration documentation present'
        if issues:
    ...
    def _test_store_memory(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'MCP tool testing requires MCP client - use Claude Desktop or MCP test client'
    ...
    def _test_cli_index(self, result: Dict):
        ...
        returncode, stdout, stderr = self._run_command([
            'python', '-m', 'src.cli', 'index', str(test_dir), '--project-name', 'test-project-cli'
        ], timeout=60)

        if not self._is_success_with_warnings(returncode, stdout, stderr):
            result['status'] = 'FAIL'
            result['notes'] = f'Index command failed with exit code {returncode}: {stderr[:300]}'
            result['bugs_found'].append({
        ...
        has_progress = any(keyword in output for keyword in ['indexed', 'files', 'processing', 'complete'])
        if not has_progress:
            issues.append({
        ...
        has_timing = any(keyword in output for keyword in ['seconds', 'time', 'duration', 'took'])

        if returncode == 0:
            result['status'] = 'PASS'
            notes_parts = [f'Index command successful - processed files in test directory']
            if file_count_match:
    ...
    def _test_cli_search(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'MANUAL TEST REQUIRED: Search command listed in help. Test actual search after indexing: python -m src.cli search "authentication" --project-name test-project. Verify: results returned, relevance scores shown, file paths and line numbers included, and search completes in <1 second.'
    ...
    def _test_cli_list(self, result: Dict):
        ...
        if self._is_success_with_warnings(returncode, stdout, stderr):
            result['status'] = 'PASS'
            result['notes'] = 'List command executed (may have warnings about Rust parser)'
        else:
            result['status'] = 'FAIL'
    ...
    def _test_cli_export(self, result: Dict):
        ...
        returncode, stdout, stderr = self._run_command([
            'python', '-m', 'src.cli', 'export', '/tmp/test_export.json'
        ], timeout=30)

        if self._is_success_with_warnings(returncode, stdout, stderr):
            result['status'] = 'PASS'
            result['notes'] = 'Export command executed (may have warnings about Rust parser)'
        else:
            result['status'] = 'FAIL'
    ...
    def _test_cli_import(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'Import command requires valid export file - manual test after export'
    ...
    def _test_cli_backup(self, result: Dict):
        ...
        returncode, stdout, stderr = self._run_command([
            'python', '-m', 'src.cli', 'backup', '/tmp/backup.tar.gz'
        ], timeout=60)
    ...
    def _test_cli_restore(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'Restore command requires valid backup - manual test after backup'
    ...
    def _test_health_score(self, result: Dict):
        ...
        result['status'] = 'MANUAL_REQUIRED'
        result['notes'] = 'Health score testing requires MCP client or CLI health command'
    ...
    def _test_api_docs(self, result: Dict):
        ...
        if api_doc.exists():
            ...
            result['status'] = 'PASS'
            result['notes'] = 'API.md not found - check if documentation exists elsewhere'
