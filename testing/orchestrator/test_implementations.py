...
''')
        ...
        semantic_ok = (
            semantic_avg < semantic_target * 5
        )  # Within 5x tolerance for Docker
...
class SecurityTests:
    ...
    def test_path_injection(self, result: Dict):
        ...
        if leaked == 0:
            result["status"] = "PASS"
            result["notes"] = (
                f"All {len(malicious_paths)} path injection attempts blocked"
            )
        else:
            result["status"] = "FAIL"
            result["notes"] = (
                f"Path injection vulnerabilities: {leaked}/{len(malicious_paths)} not blocked\n"
                + "\n".join(test_results)
            )
    ...
    def test_command_injection(self, result: Dict):
        ...
        if leaked == 0:
            result["status"] = "PASS"
            result["notes"] = (
                f"All {len(malicious_inputs)} command injection attempts blocked"
            )
        else:
            result["status"] = "FAIL"
            result["notes"] = (
                f"Command injection detected: {leaked}/{len(malicious_inputs)} attempts executed"
            )
    ...
    def test_input_validation(self, result: Dict):
        ...
        if failed == 0:
            ...
            result["status"] = "FAIL"
            result["notes"] = (
                f"Input validation issues: {failed}/{len(tests)} tests failed"
            )
...
class ErrorHandlingTests:
    ...
    def test_missing_dependencies(self, result: Dict):
        ...
        if "SERVER_STARTED" in stdout:
            ...
            actionable = (
                ...
                or "pip" in error_msg
                or "solution:" in error_msg
                or "requirements.txt" in error_msg
            )
            if actionable:
                result["status"] = "PASS"
            ...
            result["status"] = "PASS"
            result["notes"] = (
                "Server initialized successfully (all dependencies present)"
            )
    ...
    def test_invalid_inputs(self, result: Dict):
        ...
        invalid_inputs = [
            ...
            (
                "wrong_type_importance",
                'server.store_memory(content="test", category="preference", importance="high")',
            ),
            (
                "wrong_type_tags",
                'server.store_memory(content="test", category="preference", tags="not-a-list")',
            ),
        ]
        ...
        if errors_crashed == 0:
            result["status"] = "PASS"
            result["notes"] = (
                f"All {len(invalid_inputs)} invalid inputs handled gracefully"
            )
        else:
            result["status"] = "FAIL"
            result["notes"] = (
                f"Invalid input handling issues: {errors_crashed}/{len(invalid_inputs)} crashed"
            )
    ...
    def test_network_failures(self, result: Dict):
        ...
        if returncode != 0 and (
            ...
            result["status"] = "PASS"
            result["notes"] = (
                "Network failures handled (check if error messages are actionable)"
            )
        else:
            result["status"] = "FAIL"
            result["notes"] = (
                f"Network failures not handled gracefully (rc={returncode}, stdout_len={len(stdout)}, stderr_len={len(stderr)})"
            )
            result["bugs_found"].append(
                {
...
class ConfigurationTests:
    ...
    def test_env_variables(self, result: Dict):
        ...
        if all_work:
            result["status"] = "PASS"
            result["notes"] = (
                f"All {len(test_vars)} environment variables work correctly"
            )
        else:
            result["status"] = "FAIL"
    ...
    def test_model_configuration(self, result: Dict):
        ...
        if "MODEL: all-mpnet-base-v2" in stdout:
            ...
            result["status"] = "PASS"
            result["notes"] = (
                "Model configuration test - manual verification recommended"
            )
