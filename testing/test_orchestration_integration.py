...
def test_executor_integration():
    ...
    executor = TestExecutor(
        project_root="/Users/elliotmilco/Documents/GitHub/claude-memory-server"
    )
...
def test_coordinator_bug_deduplication():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        agent1_results = {
            ...
            "tests": [{"test_id": "TEST-001", "status": "FAIL", "bugs_found": []}],
            "bugs_found": [
                {
                    "id": "BUG-001",
                    "severity": "HIGH",
        ...
        agent2_results = {
            ...
            "start_time": datetime.now().isoformat(),
            "end_time": datetime.now().isoformat(),
            "tests": [{"test_id": "TEST-002", "status": "PASS", "bugs_found": []}],
            "bugs_found": [
                {
        ...
        bug_001 = next(
            b for b in coordinator.consolidated_bugs if b.get("id") == "BUG-001"
        )
        ...
        print(
            f"BUG-001 found by: {bug_001['found_by_agents']}, severity: {bug_001['severity']}"
        )
        ...
        print(
            f"Summary - Total tests: {summary['total_tests']}, Pass rate: {summary['pass_rate']:.1f}%"
        )
        print(
            f"Bugs - Critical: {summary['critical_bugs']}, High: {summary['high_bugs']}, "
        ...
        assert not readiness["ready"]  # Should not be ready (has 1 critical bug)
