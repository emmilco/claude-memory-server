...
def test_coordinator():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(coordinator.agent_results) == 3, f"Expected 3 agents, got {len(coordinator.agent_results)}"
        ...
        assert len(coordinator.consolidated_bugs) == 2, f"Expected 2 unique bugs, got {len(coordinator.consolidated_bugs)}"

        # Check that BUG-CRITICAL-001 has both agents listed
        critical_bug = next(b for b in coordinator.consolidated_bugs if b['id'] == 'BUG-CRITICAL-001')
        assert len(critical_bug['found_by_agents']) == 2, "Critical bug should be found by 2 agents"
        assert 'agent-install' in critical_bug['found_by_agents']
        assert 'agent-mcp-memory' in critical_bug['found_by_agents']
        print(f"✓ Deduplication working: 3 bug reports → 2 unique bugs")
        print(f"  - BUG-CRITICAL-001 found by {len(critical_bug['found_by_agents'])} agents")
        ...
        # Verify counts
        assert summary['total_tests'] == 9, f"Expected 9 total tests, got {summary['total_tests']}"
        assert summary['tests_passed'] == 7, f"Expected 7 passed, got {summary['tests_passed']}"
        assert summary['tests_failed'] == 2, f"Expected 2 failed, got {summary['tests_failed']}"
        assert summary['total_bugs_found'] == 2, f"Expected 2 bugs, got {summary['total_bugs_found']}"
        assert summary['critical_bugs'] == 1, f"Expected 1 critical bug, got {summary['critical_bugs']}"
        assert summary['high_bugs'] == 1, f"Expected 1 high bug, got {summary['high_bugs']}"
        ...
        assert not readiness['ready'], "Should not be production ready with critical bug"
        assert 'zero_critical_bugs' in readiness['criteria']
        assert not readiness['criteria']['zero_critical_bugs'], "Should fail zero_critical_bugs criterion"
