...
def show_status():
    ...
    for project_name, data in sorted(all_projects.items()):
        ...
        table.add_row(
            project_name, state_colored, last_activity, days_inactive, searches, files
        )
...
def export_project(
    project_name: str, output_path: str = None, include_readme: bool = True
):
    ...
    result = asyncio.run(
        exporter.export_project_archive(
    ...
    if result["success"]:
        console.print(
            f"[green]✓ Successfully exported to {result['export_file']}[/green]"
        )
...
def import_project(
    archive_path: str, project_name: str = None, conflict_resolution: str = "skip"
):
    ...
    result = asyncio.run(
        importer.import_project_archive(
    ...
    if result["success"]:
        console.print(
            f"[green]✓ Successfully imported project '{result['project_name']}'[/green]"
        )
        ...
        if result.get("conflict"):
            console.print(
                "  [yellow]Tip: Use --conflict overwrite to replace existing archive[/yellow]"
            )
...
def list_exportable():
    ...
    # Create table
    table = Table(
        title="Exportable Projects", show_header=True, header_style="bold magenta"
    )
    table.add_column("Project", style="cyan")
    ...
    for proj in sorted(result["exportable_projects"], key=lambda x: x["project_name"]):
        archived_at = (
            proj.get("archived_at", "Unknown")[:10]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Project archival and lifecycle management"
    )
    ...
    reactivate_parser = subparsers.add_parser(
        "reactivate", help="Reactivate an archived project"
    )
    reactivate_parser.add_argument("project_name", help="Name of project to reactivate")

    # Export command
    export_parser = subparsers.add_parser(
        "export", help="Export project archive to portable file"
    )
    ...
    import_parser = subparsers.add_parser(
        "import", help="Import project archive from portable file"
    )
    import_parser.add_argument("archive_path", help="Path to archive file (.tar.gz)")
    import_parser.add_argument(
        "-n", "--name", help="Custom project name (default: use name from archive)"
    )
    import_parser.add_argument(
        "--conflict",
    ...
    subparsers.add_parser(
        "list-exportable", help="List all projects available for export"
    )
