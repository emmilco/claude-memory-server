...
class BaseCallExtractor(ABC):
    ...
    @abstractmethod
    def extract_calls(
        self, file_path: str, source_code: str, parse_result: Optional[Any] = None
    ) -> List[CallSite]:
        """
...
class PythonCallExtractor(BaseCallExtractor):
    ...
    def _extract_call_site(
        self, call_node: ast.Call, caller_function: str, file_path: str
    ) -> Optional[CallSite]:
...
class JavaScriptCallExtractor(BaseCallExtractor):
    ...
    def extract_calls(
        ...
        if not self.parser:
            logger.warning(
                f"JavaScript call extraction unavailable for {file_path}: tree-sitter not initialized"
            )
            return []
    ...
    def _extract_calls_from_tree(
        ...
        if node.type in ("call_expression", "member_expression"):
            call_site = self._extract_call_from_node(
                node, source_code, file_path, current_function
            )
            if call_site:
                calls.append(call_site)
        ...
        for child in node.children:
            self._extract_calls_from_tree(
                child, source_code, file_path, calls, current_function
            )
    ...
    def _extract_call_from_node(
        self, node: Any, source_code: str, file_path: str, caller_function: str
    ) -> Optional[CallSite]:
        """
    ...
    def _extract_callee_name(self, node: Any) -> Optional[str]:
        ...
        try:
            if node.type == "identifier":
                ...
                if (
                    rightmost.type == "property_identifier"
                    or rightmost.type == "identifier"
                ):
    ...
    def extract_implementations(
        ...
        if not self.parser:
            logger.warning(
                f"JavaScript implementation extraction unavailable for {file_path}: tree-sitter not initialized"
            )
            return []
        ...
        try:
            ...
            self._extract_implementations_from_tree(
                tree.root_node, file_path, implementations
            )
    ...
    def _extract_implementations_from_tree(
        self, node: Any, file_path: str, implementations: List[InterfaceImplementation]
    ) -> None:
        """
            ...
            for child in node.children:
                if child.type == "type_identifier" or child.type == "identifier":
                    ...
                    for heritage_child in child.children:
                        if (
                            heritage_child.type == "type_identifier"
                            or heritage_child.type == "identifier"
                        ):
